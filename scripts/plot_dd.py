...
def plot_dd(input_df, output_dir):
    '''
    '''
    # set seed for reproducibility
    np.random.seed(0)

    # 5 simulations per class in the quartet
    n_sim = 5
    ...
    for sigma in sigmas:
        path = os.path.join(output_dir, 'dd' + str(n_sim) + 'x_' + str(sigma))
        ...
        for i in df.dataset:
            sim_plots[i] = dict()
            rel_df = df[df['dataset'] == i].drop('dataset', axis = 1)
        ...
        for class_name in df['dataset'].unique():
